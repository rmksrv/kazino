...
def интерпретировать(программа: Программа, допвывод: bool):
    интер = Интерпретатор.новый(программа)
    операция = программа.найти_операцию(интер.тек_инд_оп)
    while операция:
        if допвывод:
            print(f"№ операции:   {интер.тек_инд_оп}")
            print(f"Операция:     {операция.__class__.__name__}{vars(операция)}")
            print(f"Стэк:         {интер.стэк}")
            print(f"Таблица имен: {интер.таблица_имен}")
        операция.исполнить(интер)
        if допвывод:
            print("-" * 30)
            print()
        операция = программа.найти_операцию(интер.тек_инд_оп)
