...
class Токен(абц.ABC):
    ...
    # @абц.abstractmethod
    # def опы(сам, табл_им: ПромежутТаблицаИмен, табл_конст: ТаблицаКонст) -> Список[Оп]: ...
...
class Вызов(Токен):
    ...
    def генерир_опы(сам, табл_им: ПромежутТаблицаИмен, табл_конст: ТаблицаКонст) -> Список[Оп]:
        ...
        if сам.приводимое_к_двопу():
            ...
            ид_принт = табл_им.найти_или_создать("print", встроенные.print).ид
            опы.append(питон.ЗагрИмя(ид_принт))
            сам.обработать_аргументы(опы, табл_им, табл_конст)
